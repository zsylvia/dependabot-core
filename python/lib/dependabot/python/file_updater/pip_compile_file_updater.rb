  ...
  module Python
    class FileUpdater
      class PipCompileFileUpdater
        ...
        def run_pip_compile_command(opts)
          set_py_version = Shellwords.join(["pyenv", "local", python_version])
          pip_compile = Shellwords.join(%w(pyenv exec pip-compile) + opts)
          run_command("#{set_py_version} && #{pip_compile}")
        ...
        def install_required_python
          ...
          run_command(Shellwords.join(%w(pyenv install -s) + [python_version]))
          run_command(Shellwords.join(%w(pyenv exec pip install -r) +
